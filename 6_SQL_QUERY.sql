...
SELECT online_customer.CUSTOMER_ID , order_items.ORDER_ID, CONCAT(online_customer.CUSTOMER_FNAME,online_customer.CUSTOMER_LNAME) AS FULL_NAME ,SUM(order_items.PRODUCT_QUANTITY) AS TOTAL_QUANTITY FROM order_items INNER JOIN order_header ON order_items.ORDER_ID = order_header.ORDER_ID 
